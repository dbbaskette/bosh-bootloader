...
`
	...
	Describe("Generate", func() {
		...
		Context("when a CF LB is provided", func() {
			BeforeEach(func() {
				expectedTemplate = expectTemplate("vars", "bosh_director", "jumpbox", "cf_lb", "cf_instance_groups")
				expectedTemplate += "\n" + backendService + "\n" + subnetCIDRs
		...
		Context("when a CF LB is provided with a domain", func() {
			...
			It("adds the cf lb template with instance groups, backend service, and dns", func() {
				template := templateGenerator.Generate(state)
				checkTemplate(template, expectedTemplate)
			})
		})
